...
import { ArrowLeft, Package, Truck, Clock3, AlertTriangle, CheckCircle2, PlusCircle, Send, Search, ScanLine } from 'lucide-react';
import { DiscrepancyPhotoUpload } from '@/components/receiving/DiscrepancyPhotoUpload';
import { BarcodeScannerModal } from '@/components/dock/BarcodeScannerModal';
...
export default function ShipmentDetailPage() {
  ...
  const { control, handleSubmit, reset, setValue } = useForm<ReceivingFormValues>({
    defaultValues: {
      lines: {},
    },
  });
  ...
  const handleScanSuccess = (scannedCode: string) => {
    ...
    if (matchedItem) {
      const currentCount = Number(watchedLines?.[matchedItem.id]?.counted ?? 0);
      setValue(`lines.${matchedItem.id}.counted`, currentCount + 1);
      setFeedback(`Scanned ${matchedItem.sku}: incremented count by 1.`);
    } else {
      setFeedback(`Scanned code "${scannedCode}" did not match any line item SKU.`);
    }
  };
    ...
    <main className="min-h-screen bg-slate-50 p-3 md:p-8">
      <div className="mx-auto max-w-5xl space-y-4">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100"
              >
          ...
          {feedback ? (
            <div className={`rounded-xl border p-3 text-sm ${feedback.includes('completed') || feedback.includes('Scanned') ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-rose-200 bg-rose-50 text-rose-700'}`}>
              {feedback}
            </div>
          ) : null}
